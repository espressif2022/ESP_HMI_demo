 ...
 */
esp_err_t esp_lv_qoi_init(esp_lv_qoi_decoder_handle_t *ret_handle)
{
    ...
    *ret_handle = dec;
    ...
    ESP_LOGI(TAG, "qoi decoder create success, version: %d.%d.%d", ESP_LV_QOI_VER_MAJOR, ESP_LV_QOI_VER_MINOR, ESP_LV_QOI_VER_PATCH);
    return ESP_OK;
}

esp_err_t esp_lv_qoi_deinit(esp_lv_qoi_decoder_handle_t handle)
{
...
static lv_res_t decoder_info(struct _lv_img_decoder_t *decoder, const void *src, lv_img_header_t *header)
{
    ...
    if (src_type == LV_IMG_SRC_VARIABLE) {
        ...
        if (strcmp(lv_fs_get_ext(fn), "qoi") == 0) {
            uint8_t *qoi_data;          /*Pointer to the loaded data. Same as the original file just loaded into the RAM*/
...
static lv_res_t decoder_open(lv_img_decoder_t *decoder, lv_img_decoder_dsc_t *dsc)
{
    ...
    if (dsc->src_type == LV_IMG_SRC_VARIABLE) {
        ...
        if (strcmp(lv_fs_get_ext(fn), "qoi") == 0) {
            ...
            if (png_load_file(fn, &qoi_data, &qoi_data_size, false) != LV_FS_RES_OK) {
                if (qoi_data) {
                    free(qoi_data);
                    lv_qoi_cleanup(qoi);
                }
