...
bool get_wash_ui_mode(void)
{
    if (wash_mode == WASH_MODE_STANDBY) {
        return true;
    }
    return false;
}
...
static void washing_event_cb(lv_event_t *e)
{
    ...
    ESP_LOGI("washing_event_cb", "code = %d", code);
    ...
    if (wash_mode_xor ^ wash_mode) {
        ...
        case WASH_MODE_RUN: {
            if (WASH_MODE_STANDBY == wash_mode_xor) {
                ...
                lv_label_set_text_fmt(label_leftTimeH, "%ld", (wash_time_left + 59) / 3600);
                lv_label_set_text_fmt(label_leftTimeL, "%02ld", ((wash_time_left + 59) % 3600) / 60);
            }
