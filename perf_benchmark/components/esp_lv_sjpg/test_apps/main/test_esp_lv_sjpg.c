...
static const char *TAG = "SJPG test";
...
TEST_CASE("Validate SJPG decoder functionality in LVGL", "[SJPG][variable jpg]")
{
    ...
    TEST_ESP_OK(esp_lv_split_jpg_init(&sjpg_handle));

    lv_obj_t *img_decoder = lv_img_create(lv_scr_act());
    lv_obj_set_align(img_decoder, LV_ALIGN_TOP_LEFT);
    lv_img_set_src(img_decoder, &color_jpg);
    lv_refr_now(NULL);

    // Wait for flush to complete
    TEST_ASSERT_TRUE(xSemaphoreTake(flush_sem, pdMS_TO_TICKS(3 * 1000)));

    TEST_ESP_OK(esp_lv_split_jpg_deinit(sjpg_handle));
    test_lvgl_deinit(disp_drv, disp_buf);
}

TEST_CASE("Validate SJPG decoder functionality in LVGL", "[SJPG][variable sjpg]")
{
    lv_disp_drv_t *disp_drv = NULL;
    lv_disp_draw_buf_t *disp_buf = NULL;
    esp_lv_sjpg_decoder_handle_t sjpg_handle = NULL;
    ...
    lv_obj_t *img_decoder = lv_img_create(lv_scr_act());
    lv_obj_set_align(img_decoder, LV_ALIGN_TOP_LEFT);
    lv_img_set_src(img_decoder, &color_sjpg);
    lv_refr_now(NULL);
...
TEST_CASE("Validate SJPG decoder functionality in LVGL", "[SJPG][file jpg]")
{
    lv_disp_drv_t *disp_drv = NULL;
    lv_disp_draw_buf_t *disp_buf = NULL;
    esp_lv_sjpg_decoder_handle_t sjpg_handle = NULL;
    ...
    lv_obj_t *img_decoder = lv_img_create(lv_scr_act());
    lv_obj_set_align(img_decoder, LV_ALIGN_TOP_LEFT);
    lv_img_set_src(img_decoder, "A:/assets/color_jpg.jpg");
    lv_refr_now(NULL);
