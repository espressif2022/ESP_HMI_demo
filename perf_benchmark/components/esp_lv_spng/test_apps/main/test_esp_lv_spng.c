...
static const char *TAG = "SPNG test";
...
TEST_CASE("Validate SPNG decoder functionality in LVGL", "[SPNG][variable png]")
{
    ...
    TEST_ESP_OK(esp_lv_split_png_init(&spng_handle));
    ...
    TEST_ESP_OK(esp_lv_split_png_deinit(spng_handle));
    test_lvgl_deinit(disp_drv, disp_buf);
}

TEST_CASE("Validate SPNG decoder functionality in LVGL", "[SPNG][variable spng]")
{
    lv_disp_drv_t *disp_drv = NULL;
    lv_disp_draw_buf_t *disp_buf = NULL;
    esp_lv_spng_decoder_handle_t spng_handle = NULL;
    ...
    lv_obj_t *img_decoder = lv_img_create(lv_scr_act());
    lv_obj_set_align(img_decoder, LV_ALIGN_TOP_LEFT);
    lv_img_set_src(img_decoder, &color_spng);
    lv_refr_now(NULL);
...
TEST_CASE("Validate SPNG decoder functionality in LVGL", "[SPNG][file png]")
{
    lv_disp_drv_t *disp_drv = NULL;
    lv_disp_draw_buf_t *disp_buf = NULL;
    esp_lv_spng_decoder_handle_t spng_handle = NULL;
    ...
    lv_obj_t *img_decoder = lv_img_create(lv_scr_act());
    lv_obj_set_align(img_decoder, LV_ALIGN_TOP_LEFT);
    lv_img_set_src(img_decoder, "A:/assets/color_png.png");
    lv_refr_now(NULL);
