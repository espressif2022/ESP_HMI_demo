...
static const char *TAG = "QOI test";

#define TEST_LCD_H_RES      100
#define TEST_LCD_V_RES      100

LV_IMG_DECLARE(color_qoi);
...
TEST_CASE("Validate QOI decoder functionality in LVGL", "[QOI][variable qoi]")
{
    ...
    lv_obj_t *img_decoder = lv_img_create(lv_scr_act());
    lv_obj_set_align(img_decoder, LV_ALIGN_TOP_LEFT);
    // lv_img_set_src(img_decoder, &color_qoi);
    lv_refr_now(NULL);
...
TEST_CASE("Validate QOI decoder functionality in LVGL", "[QOI][file qoi]")
{
    lv_disp_drv_t *disp_drv = NULL;
    lv_disp_draw_buf_t *disp_buf = NULL;
    esp_lv_sqoi_decoder_handle_t qoi_handle = NULL;
    ...
    lv_obj_t *img_decoder = lv_img_create(lv_scr_act());
    lv_obj_set_align(img_decoder, LV_ALIGN_TOP_LEFT);
    lv_img_set_src(img_decoder, "A:/assets/color_qoi.qoi");
    lv_refr_now(NULL);
